...
export function App() {
  ...
  const showLoadMoreBtn = images.length > 0 && !loading && currentPage < totalPages;
  ...
  return (
    <>
      ...
      {loading && <Loader />}
      {error && <ErrorMessage />}
      {images.length > 0 && <ImageGallery items={images} onClickModal={onClickModal} />}
      {showLoadMoreBtn && <LoadMoreBtn onClick={loadMore} />}
      <Toaster />
      {selectedImage && (
        <ImageModal isOpen={isOpen} image={selectedImage} onCloseModal={closeModal} />
      )}
    </>
  );
}
